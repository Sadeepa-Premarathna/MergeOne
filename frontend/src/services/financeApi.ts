...
export const financeApi = {
  ...
  getFinancialOverview: async (): Promise<FinancialData> => {
    try {
      // First try to get real financial data from backend
      const data = await apiCall<FinancialData>('/finance/overview');
      return data;
    } catch (error) {
      // Fallback to mock data if API fails
      console.warn('Using fallback financial data due to API error:', error);
      return {
  ...
  getMonthlyData: async (): Promise<MonthlyData[]> => {
    try {
      const data = await apiCall<MonthlyData[]>('/finance/monthly');
      return data;
    } catch (error) {
      console.warn('Using fallback monthly data due to API error:', error);
      return [
  ...
  // Get current month expense breakdown
  getCurrentMonthExpenses: async (): Promise<ExpenseBreakdown[]> => {
    try {
      const data = await apiCall<ExpenseBreakdown[]>('/finance/expenses/current');
      return data;
    } catch (error) {
      console.warn('Using fallback expense data due to API error:', error);
      return [
  ...
  getRecentTransactions: async (): Promise<Transaction[]> => {
    try {
      const data = await apiCall<Transaction[]>('/finance/transactions/recent');
      return data;
    } catch (error) {
      console.warn('Using fallback transaction data due to API error:', error);
      return [
        {
          id: '1',
